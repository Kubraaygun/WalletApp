...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log("Yüklenen veriler:", storedData);

        if (storedData) {
          const parsedData = JSON.parse(storedData);
          console.log("İşlenen veriler:", parsedData);
          dispatch(setInitialData(parsedData));
        } else {
          console.log(
            "AsyncStorage'da veri bulunamadı, varsayılan değerler kullanılıyor."
          );
        }
      } catch (error) {
  ...
  const renderTransaction = ({ item }) => (
    <View style={styles.transactionItem}>
      <Text style={styles.transactionsText}>Alıcı: {item.receiver}</Text>
      <Text style={styles.transactionsText}>₺ {item.amount}</Text>
      <Text style={styles.transactionsText}>{formatDate(item.date)}</Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  transactionsText: {
    color: Colors.LIGHT, // Siyah arkaplan üzerinde açık renk metin
    fontSize: 16,
  },
});
