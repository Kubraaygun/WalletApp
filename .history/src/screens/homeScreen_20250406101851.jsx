...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    // Ensure balance is a valid number before animating
    if (typeof balance === "number" && !isNaN(balance)) {
      ...
      // Set default value if balance is not a valid number
      Animated.timing(animatedBalance, {
        toValue: 0,
        duration: 500,
        useNativeDriver: false,
  ...
  useEffect(() => {
    animatedBalance.addListener(({ value }) => {
      setAnimatedBalanceText(value.toFixed(2)); // Update the balance text
    });
